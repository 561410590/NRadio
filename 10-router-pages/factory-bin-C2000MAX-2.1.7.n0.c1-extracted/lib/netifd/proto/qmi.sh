...
dail_qmi_wwan_core(){
	...
	echo "acture apn config:username:[$username] password:[$password] auth:[$auth] apn:[$apn] profile:[$profile] pdptype:[$pdptype]"
...
proto_qmi_setup() {
	...
	local apn auth username password pincode delay modes pdptype profile profile_permanent dhcpv6 autoconnect plmn timeout $PROTO_DEFAULT_OPTIONS
	local ip4table ip6table	
	local ip_6 ip_prefix_length gateway_6 dns1_6 dns2_6
	json_get_vars  apn auth username password pincode delay modes pdptype profile profile_permanent dhcpv6 autoconnect plmn ip4table ip6table timeout $PROTO_DEFAULT_OPTIONS
	...
	echo "Check if request will timeout"
	local uninitialized_timeout=0
	while uqmi -s -d "$device" -t 3000 --get-pin-status | grep "Failed to connect to service" > /dev/null;do
		[ -e "$device" ] || return 1
		if [ "$uninitialized_timeout" -lt "20" ]; then
			echo "check request timeout again..."
			let uninitialized_timeout++
			sleep 1;
		else
			echo "Device request timeout"
			proto_notify_error "$interface" SIM_NOT_INITIALIZED
			return 1
		fi
	done
	echo "Waiting for SIM initialization"
	uninitialized_timeout=0
	...
	if uqmi -s -d "$device"  -t 3000 --get-pin-status | grep '"Not supported"\|"Invalid QMI command"' > /dev/null; then
		[ -n "$pincode" ] && {
