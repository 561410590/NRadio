...
PHY_STATUS_DIR="/var/run/phy"

wait_phy_restart(){
	local wait_down_cnt=0
	local unormal=0

	while true;do
		if cpetools.sh -i "$interface" -c check_qmap_dail ;then
			echo "qmap wwan online"
			unormal=1
			break
		fi
		sleep 1
		...
		if [ $wait_down_cnt -gt 8 ];then
			echo "qmap wait wwan,timeout"
			break
		fi		
	done
...
proto_ncm_setup() {
	...
	for i in $initialize; do
		...
		if echo "$i" |grep -qs "nat" ;then
			cf_background=$(uci -q get network.$interface.background)
	...
	if [ "$vendor" == "quectel_qualcomm" ];then
		if wait_phy_restart ;then
