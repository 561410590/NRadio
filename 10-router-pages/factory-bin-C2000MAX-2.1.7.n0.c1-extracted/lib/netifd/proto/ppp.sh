...
proto_pptp_setup() {
	local config="$1"
	local iface="$2"
	local mppe_disabled=$(uci -q get network.vpn.pptp_mppe_disabled)
	...
	[ ! -d "/tmp/etc/ppp" ] && mkdir -p "/tmp/etc/ppp"
	cp -f /etc/ppp/options.pptp /tmp/etc/ppp/options.pptp
	if [ "$mppe_disabled" = "1" ]; then
		sed -i "/mppe/cnomppe" "/tmp/etc/ppp/options.pptp"
	fi
	ppp_generic_setup "$config" \
		plugin pptp.so \
		pptp_server $server \
		file /tmp/etc/ppp/options.pptp
}
