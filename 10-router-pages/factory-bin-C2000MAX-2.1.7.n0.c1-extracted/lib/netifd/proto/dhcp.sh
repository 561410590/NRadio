...
restart_dhcp_process(){
	...
	json_close_object
	echo "force $interface dhcp restart"
...
proto_dhcp_teardown() {
	...
	local iperrorcnt=$(ubus call network.interface.$INTERFACE_PREFIX status|jsonfilter -e '$["data"]["iperrorcnt"]')
	proto_kill_command "$interface"
	if  echo  "$INTERFACE_PREFIX"|grep "cpe" && [ -z "$iperrorcnt" -a "$interface" == "${INTERFACE_PREFIX}_4" -a "$status" == "true" ];then
		$(restart_dhcp_process "$interface")
	fi
}
add_protocol dhcp
