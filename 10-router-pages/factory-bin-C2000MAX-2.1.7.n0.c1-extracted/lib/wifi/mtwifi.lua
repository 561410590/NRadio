...
local function add_vif_into_net(uci_vif)
    local vif = uci_vif["ifname"] or uci_vif[".name"]
    local net = uci_vif.network
    local hairpin = uci_vif.hairpin
    local phy = vif == "rai0" and "phy1" or "phy0"
    ...
    if uci_vif.mode == "ap" then
        if not hairpin or (hairpin and hairpin == "1") then
            os.execute("mwctl phy "..phy.." set hairpin_mode 1 2>/dev/null && bridge link set dev "..vif.." hairpin on 2>/dev/null")
        else
            os.execute("mwctl phy "..phy.." set hairpin_mode 0 2>/dev/null && bridge link set dev "..vif.." hairpin off 2>/dev/null")
        end
    end
end
...
function mtwifi_up(devname)
    ...
    if l1parser and devs then
        ...
        local uci_dev = mtkdat.get_uci_dev_by_dev_name(uci, devname)
    ...
    if not devname then
        ...
        if qsetting then
            for _devname, _dev in mtkdat.spairs(devs.devname_ridx) do
                ...
                if _dev.normal_reload == true then
                    normal_reload = true
                end
