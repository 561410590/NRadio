...
local country_regions = {
    CN = {region = 1, aregion = 0, aregion_nodfs = 0, rd_region = "CE"},
    US = {region = 0, aregion = 9, aregion_nodfs = 10, rd_region = "FCC"},
    CA = {region = 0, aregion = 9, aregion_nodfs = 10, rd_region = "FCC"},
    DE = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    FR = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    GB = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    IT = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    ES = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    NL = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    BE = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    AT = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    CH = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    CZ = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    DK = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    FI = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    GR = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    HU = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    IE = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    LU = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    LV = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    MT = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    PL = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    PT = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    RO = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    SK = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    SI = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    SE = {region = 1, aregion = 1, aregion_nodfs = 6, rd_region = "CE"},
    JP = {region = 5, aregion = 1, aregion_nodfs = 6, rd_region = "JAP"},
    KR = {region = 1, aregion = 7, aregion_nodfs = 10, rd_region = "CE"},
    ID = {region = 1, aregion = 4, aregion_nodfs = 4, rd_region = "CE"}
}
...
function mtkdat.get_uci_dev_by_dev_name(uci, devname)
    for _, dev in pairs(uci["wifi-device"]) do
        if dev[".name"] == devname or dev["devname"] == devname then
            return dev
        end
    end
    return nil
...
function mtkdat.get_uci_vif_by_vif_name(uci, vifname)
    for _, vif in pairs(uci["wifi-iface"]) do
        if vif[".name"] == vifname or vif["ifname"] == vifname then
            return vif
        end
    end
    return nil
...
local uci_dev_options = {
    ...
    "dfs_slave", "cp_support", "band4_dfs_enable", "dfs_zero_wait_bandidx", "devname", "hwmode", "phyname"
}
...
local uci_iface_options = {
    ...
    "macaddr", "proxy_arp", "ht_mpdu_density", "ifname", "macfilter", "maclist"
}
...
--[[
    ...
    elseif encryption == "psk-mixed+tkip" then
        auth = "WPAPSK,WPA2PSK"
        encr = "TKIP"
    elseif encryption == "psk-mixed+tkip+ccmp" or encryption == "psk-mixed" then
        auth = "WPAPSK,WPA2PSK"
        encr = "TKIPAES"
...
local function cfg2dev(cfg, devname, dev, n)
    assert(cfg ~= nil)
    assert(dev ~= nil)
    ...
    dev[".name"] = "radio"..(n-1)
    dev.devname = devname
    dev.phyname = devname:lower():match("([^%.]+)")
    dev.type = "mtkwifi"
    dev.vendor = "mediatek"
...
--        dev.bw = "20"
    ...
    if dev.htmode:match("^EHT") then
        dev.hwmode = "11be"
    elseif dev.htmode:match("^VHT") then
        dev.hwmode = "11ac"
    elseif dev.htmode:match("^HE") then
        dev.hwmode = "11ax"
    elseif dev.htmode:match("^HT") then
        dev.hwmode = "11n"
    end
...
local function cfg2iface(cfg, devname, ifname, iface, i, n)
    assert(cfg ~= nil)
    assert(iface ~= nil)
...
--    iface.wireless_mode = token_get(cfg.WirelessMode, i, mtkdat.split(cfg.WirelessMode,";")[1])
    ...
    if iface.access_policy == "2" then
        iface.macfilter = "deny"
    elseif iface.access_policy == "1" then
        iface.macfilter = "allow"
    else
        iface.macfilter = "none"
    end
    iface.maclist = iface.access_list
...
--    cfg.IEEE80211H = dev.ieee80211h
    ...
    if country_regions[dev.country] then
        cfg.RDRegion = country_regions[dev.country].rd_region
        cfg.CountryRegion = country_regions[dev.country].region
        if dev.dfs_enable == "1" then
            cfg.CountryRegionABand = country_regions[dev.country].aregion
        else
            cfg.CountryRegionABand = country_regions[dev.country].aregion_nodfs
        end
    else
...
--    cfg.WirelessMode = token_set(cfg.WirelessMode, i, iface.wireless_mode)
    ...
    if iface.macfilter == "deny" then
        iface.access_policy = "2"
    elseif iface.macfilter == "allow" then
        iface.access_policy = "1"
    else
        iface.access_policy = "0"
    end
    iface.access_list = iface.maclist
...
function mtkdat.dat2uci()
    ...
    for devname, profile in mtkdat.spairs(profiles, function(a,b) return string.upper(a) < string.upper(b) end) do
        ...
        while i <= if_num do
            ...
            cfg2iface(cfg, dev[".name"], ifname, iface, i, n)
    ...
    if mode == "ap" then
        ...
        for idx, vifname in pairs(ifaces) do
            ...
            if vif ~= nil and vif.mode == "ap" and string.find(mldgroup._device_list, vif.device) == nil then
                ...
                local devname = ucicfg[vif.device].devname
                cfg = cfgs[devname]
    ...
    for _, dev in pairs(uci["wifi-device"]) do
        ...
        for _, iface in pairs(vifs) do
            if iface.mode == 'ap' then
                ...
                local i = string.find(iface["ifname"], "%d+")
                -- i = string.sub(iface[".name"], i, -1)
                i = string.sub(iface["ifname"], i, -1)
                i = tonumber(i) + 1
