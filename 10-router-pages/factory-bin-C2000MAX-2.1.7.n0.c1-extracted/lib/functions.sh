...
# added by NRadio
model_name() {
	[ -e /tmp/sysinfo/model ] && cat /tmp/sysinfo/model || echo "generic"
}
...
softver() {
	local ver_str="$1"
	if [ -z "$ver_str" ]; then
		ver_str=$(cat /etc/openwrt_version)
	fi
	local main_ver=$(echo $ver_str|cut -d '.' -f 1)
	local minor_ver=$(echo $ver_str|cut -d '.' -f 2)
	local build_ver=$(echo $ver_str|cut -d '.' -f 3)
	local version=$(printf "%d%03d%05d" $main_ver $minor_ver $build_ver)
