...
local function get_ip(mac)
	...
	for line in file:lines() do 
		local _ip, _mac = line:match('^(%d+%.%d+%.%d+%.%d+).*(%x%x:%x%x:%x%x:%x%x:%x%x:%x%x)')
		if _ip and _mac and _mac == mac then
			ip = _ip
			break
		end
	end
...
local function http_404(task)	
	...
	if auth.neterr and type(auth.neterr) == "string" then
...
local function init_list_f(data,reset)
	if not data then
		return
	end
	if reset == 1 then
...
local function http_wifidog(task, urlpath)
	...
	elseif node == 'authed' then
		log.info('recv authed')
		local clean = (task.req.params or NULL).clean
		local succeed = auth.get_result("firststart")
		if succeed then
			local cjsonx = require "cjson.safe"
			local cache_data = cjsonx.encode(succeed)
			if cache_data then
				log.info('start cache_data: ', cache_data)
			end
			if clean == "1" then
				init_list_f(succeed,1)
			else
				init_list_f(succeed,0)
			end
			http_error(ch, 200, nil, '{"result":"OK"}')
		end
	elseif node == 'close' then
		log.info('recv close')
		fw.allow_skip()
		http_error(ch, 200, nil, '{"result":"OK"}')
