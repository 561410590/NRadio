...
local function init_url(url)
	...
	if https then
		...
		tasklet.sslstream_channel.ctx = require('ssl2').context.new('sslv23')
...
local function make_reqdata(reqarr, n)
	...
	for i = 1, n do 
		...
		elseif req == 'firststart' then
			reqdata.firststart = true
		elseif req.mac then
