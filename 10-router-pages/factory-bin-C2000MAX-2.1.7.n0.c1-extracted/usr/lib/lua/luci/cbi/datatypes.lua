...
local tonumber, find,tostring, type, unpack, select = tonumber,string.find, tostring, type, unpack, select
...
function multimacaddr(val)
	local parts
	if find(val,"\r\n") then
		parts = util.split( val, "\r\n" )
	else
		parts = util.split( val, "\n" )
	end
	for i = 1,#parts do		
		if #parts[i] > 0 then
			if macaddr(parts[i]) == false then
				return false
			end
		end
	end
...
function timehhmm(val)
	local hhstr, mmstr = val:match("^(%d+):(%d+)$")
	if (hhstr == nil) or (mmstr == nil) then
		return false
	end

	local hh = tonumber(hhstr);
	local mm = tonumber(mmstr);
