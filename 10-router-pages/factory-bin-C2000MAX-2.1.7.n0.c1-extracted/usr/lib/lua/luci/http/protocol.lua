...
function clean_upload_temp()
	local image_head = luci.sys.getenv("HTTP_TEMPPOST_BODYHEAD")
	if image_head and nixio.fs.access(image_head) then                                       
		os.execute("rm "..image_head.." > /dev/null")
	end 
end

function get_server_post_file()
	local image_tmpfile = luci.sys.getenv("HTTP_TEMPPOST_FILENAME")
	if image_tmpfile and nixio.fs.access(image_tmpfile) then 
		return image_tmpfile
	end
	return ""
end
...
function mimedecode_message_body( src, msg, filecb,filetarget )
		...
		if stat > 0 then
			if field.headers["Content-Disposition"] then
				if field.headers["Content-Disposition"]:match("^form%-data; ") then
					...
					if field.file then
						image_target = "/tmp/"..field.file
					end
					nginx_post_file = get_server_post_file()
					if #nginx_post_file > 0 then
						if filetarget then
							image_target = filetarget
						end	
					end
				end
			end
			...
			if field.name and field.file and filecb then
				...
				if #nginx_post_file > 0 then
					nixio.syslog("err","reset1 ,mv "..nginx_post_file.." to "..image_target)
					os.execute("mv "..nginx_post_file.." "..image_target.." > /dev/null")
				else
					store = filecb
				end
...
function parse_message_body( src, msg, filecb,filetarget )
	-- Is it multipart/mime ?
	if msg.env.REQUEST_METHOD == "POST" and msg.env.CONTENT_TYPE and
	   msg.env.CONTENT_TYPE:match("^multipart/form%-data")
	then
		local deal_result = mimedecode_message_body( src, msg, filecb,filetarget )
		if luci.sys.getenv("HTTP_TEMPPOST_FILENAME") then
			clean_upload_temp()
		end
		return deal_result
