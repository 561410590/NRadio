...
local pduString = require("luci.luapdu.string")
...
function pduSmsObject.newTx(recipientNum, content)
    if recipientNum and not recipientNum:match("^\+?%d+$") then
        nixio.syslog("err","Invalid recipient number! <"..recipientNum..">")
    end
    local content = {
            msgReference=0,
            recipient={
...
function pduSmsObject:encode16bitPayload(content)
    ...
    while pduString:len(content) ~= 0 do
        ...
        elseif byte <= 0xEF then    -- 16bit
            ...
            return nixio.syslog("err","Can't fit payload char into 16bit unicode!")
        end
        length = length + 2
    end
...
function pduSmsObject:encode7bitPayload(content)
    ...
    while pduString:len(content) ~= 0 or carryover ~= 0 do
        local char = content:sub(1,1)
        if pduString:len(content) ~= 0 then length = length + 1 end
        local charval = encodeTable7bit[char]
        content = content:sub(2)
        if charval == nil then charval = 0 end
        local val = bit.lshift(charval, state) + carryover
        if state~= 0 or pduString:len(content) == 0 then
            response[#response+1] = pduString:octet(bit.band(val, 0xFF))
...
function pduSmsObject:encodePayload(alphabetOverride)
    ...
    elseif alphabetOverride ~= 8 and
        ...
        nixio.syslog("err","Invalid alphabet override!")
    end
    ...
    for i,part in ipairs(content) do
        ...
        if alphabetOverride == 4 then
            while pduString:len(part) ~= 0 do
                local byte1 = part:byte(1)
                part = part:sub(2)
                if byte1 > 127 then
                    local byte2 = part:byte(1)
                    part = part:sub(2)
                    asciiByte = bit.band( 0xFF, bit.rshift(byte1,6) +
                                                bit.band(byte2,0x3F))
        ...
        elseif alphabetOverride == 0 then
            text, length = self:encode7bitPayload(part)
            if self.msg.multipart and length > 0 then
                length = length + 1
                if length%2 == 1 then
                    text[#text+1]="00"
                end
            ...
            nixio.syslog("err","Unimplemented payload encoding alphabet!")
        end
...
function pduSmsObject:contentProcessing()
    ...
    local contentLen = pduString:len(self.msg.content)
    local charWidth = 7
    if     self.dcs == 8 then charWidth = 16
    elseif self.dcs == 4 then charWidth =  8 end
    local maxChars = 140*8/charWidth

    nixio.syslog("err","contentLen:"..contentLen)
    nixio.syslog("err","maxChars:"..maxChars)
    self.msg.multipart = contentLen > maxChars

    --[[if self.msg.multipart then --no choose ,avoid model bug
        if self.dcs == 0 then
            self.dcs = 8
            charWidth = 16
            maxChars = 140*8/charWidth
            nixio.syslog("err","fix maxChars:"..maxChars)
        end
    end--]]
    ...
    if self.msg.multipart then
        ...
        if self.msg.partCount > 255 then
            nixio.syslog("err","Message content too long!")
            return
        end
        self.msg.parts = {}
        local content = self.msg.content
        while pduString:len(content) > 0 do            
            self.msg.parts[#self.msg.parts+1] = pduString:sub(content,1,maxChars) --!todo >7bit characters take up more than 8 bits, so this vulgar method can accidentally split a LUA unicoded char in two
            content = pduString:sub(content,maxChars+1)
        end
    else
...
function pduSmsObject:encode()
    ...
    elseif self.recipient then
        ...
        nixio.syslog("err","No valid content!")
    end
    local pduParts = {}
