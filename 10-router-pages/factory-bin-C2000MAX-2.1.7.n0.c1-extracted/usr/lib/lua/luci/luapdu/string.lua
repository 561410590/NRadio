...
function pduString:len(inputstr)
...
function pduString:sub(inputstr,startindex,endindex)
    if not inputstr or type(inputstr) ~= "string" or #inputstr <= 0 then
        return ""
    end
    local length = 0  -- 字符的个数
    ...
    local targetstr=""
    local targetstart=1
    local targetend=string.len(inputstr)
    local start_found = 0
    while true do
        local curByte = string.byte(inputstr, i)
        local byteCount = 1
        ...
        if endindex and endindex == length then
            targetend=i+ byteCount - 1
            break
        end
        i = i + byteCount
    ...
    if targetend >= startindex then        
        targetstr=string.sub(inputstr, targetstart, targetend)
        return targetstr
    end
    return inputstr:sub(startindex,endindex)
end
...
function pduString:decodeOctet(str)
    if pduString:len(str) < 2 then nixio.syslog("err","Too short, can't get octet!") end
    return tonumber("0x"..str:sub(1,2)), str:sub(3)
end

function pduString:decodeDecOctets(str, count)
    if pduString:len(str) < count*2 then nixio.syslog("err","String too short!") end
    -- Flip the octets
    local result = ""
    local var = str:sub(1,count*2)
    while pduString:len(var) ~= 0 do
...
function pduString:octet(val)
    if val == nil then val = 0; nixio.syslog("err","NIL!")
    elseif val > 255  then nixio.syslog("err","Can't convert to octet - value too large!") end
...
function pduString:decOctets(val)
    local response = {}
    val = val or ""
    if pduString:len(val) % 2 ~= 0 then val = val .. "F" end
    while pduString:len(val) ~= 0 do
...
function pduString:decodePayload(content, dcs, length)
    ...
    elseif dcsBits == 12 then nixio.syslog("err","Invalid alphabet size!") end
end
...
function pduString:decode7bitPayload(content, length)
    ...
    local state = 0
    local octet
    while pduString:len(content) ~= 0 and length ~= 0 do
    ...
    if     length     ~= 0 then
        nixio.syslog("err","Content shorter than expected!")
    elseif pduString:len(content) ~= 0 then
        nixio.syslog("err","Content longer than expected!<"..content..">")
    end
...
function pduString:decode16bitPayload(content, length)
    ...
    if     length        ~= 0 then
        nixio.syslog("err","Content shorter than expected!<"..length..">")
    elseif pduString:len(content) ~= 0 then
        nixio.syslog("err","Content longer than expected!<"..content..">")
    end
    return table.concat(data)
...
function pduString:decodeTXmsg(content, response)
    ...
    if bit.band(response.type, 0x40) == 0x40 then
        response.msg.len = response.msg.len - 6
        content = content:sub(13)
...
function pduString:decodePDU()
    ...
    elseif typeBits == 1 then
        ...
        nixio.syslog("err","Unknown message type!")
    end
end
