...
function user.setpasswd(username, password)
	...
	return os.execute(
		...
		"passwd -a md5 '" .. username .. "' >/dev/null 2>&1"
	)
end
...
function wifi_cache()
	...
	_uci:foreach("wireless", "wifi-device",
		function(s)
			radios[s['.name']] = { up = true, disabled = false, interfaces = {} }
			if s['disabled'] ~= nil and s['disabled'] == '1' then
				radios[s['.name']].up = false
				radios[s['.name']].disabled = true
			end
		end
	)
	_uci:foreach("wireless", "wifi-iface",
		function(s)
			local radio = radios[s['device']]
			if radio then
				radio.interfaces[#radio.interfaces + 1] = { section = s['.name'], ifname = s['ifname'] }
			end
		end
	)
