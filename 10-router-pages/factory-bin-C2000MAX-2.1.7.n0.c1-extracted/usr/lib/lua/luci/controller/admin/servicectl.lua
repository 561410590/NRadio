...
function index()
	entry({"servicectl"}, alias("servicectl", "status"), nil, nil, true).sysauth = "root"
	entry({"servicectl", "status"}, call("action_status"), nil, nil, true).leaf = true
	entry({"servicectl", "restart"}, post("action_restart"), nil, nil, true).leaf = true
end
