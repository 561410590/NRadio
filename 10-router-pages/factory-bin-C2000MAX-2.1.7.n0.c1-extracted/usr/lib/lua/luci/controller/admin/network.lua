...
--	if page.inreq then
		...
		if has_switch then
			page = entry({"admin", "network", "vlan"}, cbi("admin_network/vlan"), _("Switch"), 20)
		...
		page = entry({"admin", "network", "iface_status"}, call("iface_status"), nil, nil,true)
		page.leaf = true
		...
		if nixio.fs.access("/etc/config/dhcp") then
			page = entry({"admin", "network", "dhcp"}, cbi("admin_network/dhcp"), _("DHCP and DNS"), 30)
			-- page = node("admin", "network", "dhcp")
			-- page.target = cbi("admin_network/dhcp")
			-- page.title  = _("DHCP and DNS")
			-- page.order  = 30

			page = entry({"admin", "network", "dhcplease_status"}, call("lease_status"), nil, nil, true)
			page.leaf = true

			page = entry({"admin", "network", "hosts"}, cbi("admin_network/hosts"), _("Hostnames"), 40)
		...
		page = entry({"admin", "network", "routes"}, cbi("admin_network/routes"), _("Static Routes"), 50, true)
		-- page  = node("admin", "network", "routes")
		-- page.target = cbi("admin_network/routes")
		-- page.title  = _("Static Routes")
		-- page.order  = 50

		page = entry({"admin", "network", "diagnostics"}, cbi("admin_network/diagnostics"), _("Diagnostics"), 60)
		-- page = node("admin", "network", "diagnostics")
		-- page.target = template("admin_network/diagnostics")
		-- page.title  = _("Diagnostics")
		-- page.order  = 60

		page = entry({"admin", "network", "diag_ping"}, post("diag_ping"), nil, nil, true)
		page.leaf = true

		page = entry({"admin", "network", "diag_nslookup"}, post("diag_nslookup"), nil, nil, true)
		page.leaf = true

		page = entry({"admin", "network", "diag_traceroute"}, post("diag_traceroute"), nil, nil, true)
		page.leaf = true

		page = entry({"admin", "network", "diag_ping6"}, post("diag_ping6"), nil, nil, true)
		page.leaf = true

		page = entry({"admin", "network", "diag_traceroute6"}, post("diag_traceroute6"), nil, nil, true)
		page.leaf = true
--	end
end
...
function diag_ping(addr)
	...
	nixio.syslog("err","WEB["..client_ip.."] ping "..addr.." src:"..src)
	if #src == 0 then
		diag_command("ping -c 5 -W 1 %q 2>&1", addr)
	else
		diag_command("ping -I "..src.." -c 5 -W 1 %q 2>&1", addr)
	end
end

function diag_traceroute(addr)
	local src = luci.http.formvalue("src") or ""
	nixio.syslog("err","WEB["..client_ip.."] traceroute "..addr.." src:"..src)
	if #src == 0 then
		diag_command("traceroute -q 1 -w 1 -n %q 2>&1", addr)
	else
		if luci.ip.IPv4(src) then
			diag_command("traceroute -s "..src.." -q 1 -w 1 -n %q 2>&1", addr)
		else
			diag_command("traceroute -i "..src.." -q 1 -w 1 -n %q 2>&1", addr)
		end
	end
end

function diag_nslookup(addr)
	nixio.syslog("err","WEB["..client_ip.."] nslookup "..addr)
	diag_command("nslookup %q 2>&1", addr)
end

function diag_ping6(addr)
	nixio.syslog("err","WEB["..client_ip.."] ping6 "..addr)
	diag_command("ping6 -c 5 %q 2>&1", addr)
end

function diag_traceroute6(addr)
	nixio.syslog("err","WEB["..client_ip.."] traceroute6 "..addr)
