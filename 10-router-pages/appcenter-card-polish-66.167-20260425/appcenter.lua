...
local function read_file_first_line(path)
...
local function read_cpu_stat()
	local line = read_file_first_line("/proc/stat")
	if not line then
		return nil
	end
...
local function read_cpu_usage_percent()
	local fs = require "nixio.fs"
	local state_path = "/tmp/nradio_appcenter_cpu.stat"
	local total, idle = read_cpu_stat()
...
local function read_temperature_celsius()
	local fs = require "nixio.fs"
	local max_temp = nil
	for i = 0, 8 do
		local path = string.format("/sys/class/thermal/thermal_zone%d/temp", i)
		if fs.access(path) then
			local raw = tonumber(read_file_first_line(path) or "")
			if raw then
				local temp = raw
				if temp > 1000 then
					temp = temp / 1000
...
local function read_system_memory()
...
function action_sys_status()
	local load_line = read_file_first_line("/proc/loadavg") or ""
	local load1 = load_line:match("^(%S+)") or "-"
	local mem_total, mem_used, mem_percent = read_system_memory()

	luci.nradio.luci_call_result({
		cpu_percent = read_cpu_usage_percent(),
		cpu_temp = read_temperature_celsius(),
		mem_total = mem_total,
		mem_used = mem_used,
